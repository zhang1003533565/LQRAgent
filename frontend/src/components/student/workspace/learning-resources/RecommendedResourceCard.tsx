...
import type { RecommendedResource } from '@/utils/types/learningResources'
...
export function RecommendedResourceSection({
  ...
  onResourceClick?: (id: string, resource: RecommendedResource) => void
  onViewMore?: () => void
}) {
  return (
    <section className="rounded-[18px] border border-[#E6EEFA] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-[#64748B]">基于当前学习路径与资源缺口推荐</p>
        </div>
        {resources.length > 4 ? (
          <button
            type="button"
      ...
      {resources.length === 0 ? (
        <p className="rounded-xl border border-dashed border-[#D8E8FF] bg-[#F8FBFF] px-4 py-8 text-center text-sm text-[#64748B]">
          暂无推荐资源。完成路径节点或点击「AI 生成」创建第一份资料。
        </p>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {resources.map((item) => (
            <RecommendedResourceCard
              key={item.id}
              resource={item}
              onClick={() => onResourceClick?.(item.id, item)}
            />
          ))}
        </div>
      )}
