...
import type { RecentLearningItem } from '@/utils/types/chatSidebar'
...
export default function RecentLearningCard({ items, loading }: Props) {
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col rounded-[18px] border border-[#E6EEFA] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
      ...
      {loading ? (
        <div className="flex flex-1 items-center justify-center text-sm text-[#94A3B8]">加载中…</div>
      ) : items.length === 0 ? (
        <div className="flex flex-1 items-center justify-center px-2 text-center text-sm text-[#94A3B8]">
          完成练习、上传资料或推进路径后，这里会显示最近动态
        </div>
      ) : (
      ...
      <button
        type="button"
        onClick={() => navigate('/workspace/quiz')}
