...
> **版本**：v1.3  
> **日期**：2026-06-23  
> **状态**：Sprint 1–4 已落地；前端对接与 CI 意图全量回归为后续项  
...
| Pipeline 断点重试 | ✅ | `failed_step` + `executeFromStep` + retry API |
| learning_loop 测试对齐 | ✅ | 测试 API 与 `QuizService` 共用 `executeLearningLoop()` |
| LearnerContextService | ✅ | profile + mastery + memory → Planning / Pipeline |
| 意图回归 | ✅ | `IntentSuiteRunner` + `PlanningAgentIntentTest` + CI workflow |
...
| 意图全量回归需 LLM | CI 默认只跑快通道单测；全量设 `LQR_INTENT_SUITE=true` | P2 |
...
| learning_loop | `POST /api/test/learning-loop` | 四步闭环，与线上一致 |
| 学习者上下文 | `GET /api/test/learner-context` | profile + mastery + memory |
...
### Sprint 4：Pipeline 可靠 + 学习闭环（3–5 天）✅ 已落地

#### 6.4.1 Pipeline 失败重试 ✅

#### 6.4.2 learning_loop 测试对齐 ✅

`QuizService.executeLearningLoop()` 与 `/api/test/learning-loop`、`/api/test/quiz-submit`（默认 `mode=loop`）共用。

#### 6.4.3 意图回归测试集 ✅

- `IntentSuiteDefinitions` / `IntentSuiteRunner` / `PlanningAgentIntentTest`（CI 默认）
- `PlanningAgentIntentIntegrationTest`（设 `LQR_INTENT_SUITE=true` 跑全量 LLM）
- `.github/workflows/backend-ci.yml`

#### 6.4.4 LearnerContextService ✅

`GET /api/test/learner-context?userId=1`；`planOnly` / `handleMessage` / Pipeline `TaskContext` 注入学习者摘要。
