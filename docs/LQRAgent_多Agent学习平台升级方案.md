...
| AgentCardRegistry | ✅ | 13/20 Agent 已实现 `getAgentCard()` |
| Artifact 模型 | ✅ | `Artifact` / `ArtifactKind` / `AgentResponse.artifacts` |
| QualityGate | ✅ | 已接入 PipelineEngine |
| PipelineTask 持久化 | ✅ | DB + REST 查询 API |
| learning_loop | ✅ | QuizService 异步触发 |
| ai-server 工具 | 🔧 | QaAgent/QuizAgent/DiagramAgent 已接 Tool；部分改动未提交 |
...
| 问题 | 影响 | 优先级 |
|------|------|--------|
| **测试观测层缺失** | 改一处需靠前端聊天验证 | P0 |
| `ChatRouteDispatcher` 仍用 `agentId.contains` | Artifact 协议未贯通 WS | P0 |
| CapabilityRegistry 与 AgentCardRegistry 双轨 | HELP/能力目录可能不一致 | P1 |
| 7 个 Agent 使用默认 AgentCard | PlanningAgent 规划时几乎不会选中 | P1 |
| `/api/test/quiz-submit` 未走 learning_loop | 测试与线上闭环不一致 | P1 |
| 前端 `pipelineTaskApi` 零引用 | 刷新丢失 Pipeline 进度 | P1（前端） |
| `requestPeer` 无调用 | A2A 名存实亡（可接受） | P2 |
| 单元测试仅 1 个文件 | 回归风险高 | P1 |
| Pipeline retry API 未实现 | 失败后只能全量重跑 | P1 |

### 4.3 AgentCard 覆盖明细

**已实现 getAgentCard（13 个）**：QaAgent、QuizAgent、LearningPathAgent、ProfileAgent、ContentAnalysisAgent、PromptGenAgent、MediaGenAgent、QualityAgent、AssessmentAgent、EffectAgent、RecommendationAgent、SummaryAgent、DiagramAgent

**仍用默认兜底卡片（7 个）**：LessonAgent、ResourceAgent、DifficultyAgent、KnowledgeStateAgent、LearningStyleAgent、SpacedRepetitionAgent、InterventionAgent

### 4.4 现有测试入口

| 入口 | 路径 | 局限 |
|------|------|------|
| Agent 测试 API | `/api/test/agent` | 无 plan 步骤、无 artifacts |
| 管理后台 | `POST /admin/agent-test` | 同上 |
| 启动控制台 | `ConsoleControlPanel` | 仅 config/users/upload/ai ping |
| WebSocket | 聊天 | 完整链路但难自动化回归 |
...
### Sprint 3：AgentCard 补全 + 删除双轨（2–3 天）
...
| 删除类 | capability/CapabilityRegistry.java |
| 更新测试 | CapabilityRegistryTest.java → AgentCardRegistryTest |
...
```
agents          → ≥20 条，无 "Agent xxx_agent" 默认描述
...
| 文件 | Sprint |
|------|--------|
| capability/CapabilityRegistry.java | 3 |
| AgentResponse 的 @Deprecated 四字段 | 3 完成后 |
...
| **当前完成度** | 后端 ~70%；测试层 ~10%；前端协议对齐 ~50% |
| **最大缺口** | 测试可观测性、Artifact WS 贯通、AgentCard 补全 |
| **实施策略** | ✅ 先测后改：Sprint 1 不动业务，先让链路可见 |
| **文档一致性** | ARCHITECTURE_UPGRADE.md 仍写 Vue/TS，应以本文档 + 实际 React 为准 |

**定稿：可按本文档 Sprint 2 继续实施。**

---

## 附录 C：Sprint 1 落地记录（2026-06-23）
...
│   ├── handler/ChatRouteDispatcher.java  # 待重构
│   └── proxy/AiServerWsProxy.java
├── agents/                            # 20 个业务 Agent
├── admin/controller/
│   ├── AgentTestController.java       # 待扩展
│   └── ConsoleControlPanel.java       # 待扩展
