...
-- 数据库：agent
-- 说明：
--   1. JPA 只会为带 @Entity 的类建表（当前仅 sys_user、kb_upload_task），
--      因此不会自动出现 learner_profile、chat_message 等表。
--   2. 推荐：执行本脚本，或启动后端（会自动运行 resources/db/init-tables.sql）。
--   3. 所有字段均已添加中文 COMMENT，便于在数据库工具中查看。
-- ============================================================
...
-- ============================================================
-- 0. 增量补丁（须先于全量 DDL，保证旧库无损升级）
-- 新库首次初始化时本节多为 no-op，随后 CREATE IF NOT EXISTS 建表。
-- ============================================================

-- 规避 MySQL 保留字：user → sys_user（旧表存在且新表不存在时）
SET @rename_user := (
...
-- learning_path_step.status（列不存在时追加）
SET @add_step_status := (
...
-- ============================================================
-- 初始数据说明
-- 账号由 DataInitializer.java 在应用启动时自动创建，无需手动插入。
-- 默认账号：
--   admin    / 123456 → ADMIN
--   student1 / 123456 → STUDENT
-- MySQL 连接密码（application.properties）：123456
-- ============================================================
