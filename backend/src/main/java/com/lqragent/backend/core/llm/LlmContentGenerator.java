...
public class LlmContentGenerator {
     ...
     * 调用 LLM 对学习路径做个性化排序。
     *
     * @param kpIds         当前路径的知识点ID列表（拓扑序）
     * @param profileHint   学生画像摘要文本
     * @return 排序后的 kp_id 列表，失败时返回 null（调用方保留原顺序）
     */
    @SuppressWarnings("unchecked")
    public List<String> generatePathSort(List<String> kpIds, String profileHint) {
        String host = runtimeConfig.get(ConfigKeys.LLM_HOST);
        String apiKey = runtimeConfig.get(ConfigKeys.LLM_API_KEY);
        String model = runtimeConfig.get(ConfigKeys.LLM_MODEL, "gpt-4o-mini");

        if (host == null || host.isBlank() || apiKey == null || apiKey.isBlank()) {
            log.warn("[LlmContentGenerator] LLM 未配置，跳过路径排序");
            return null;
        }

        String systemPrompt = SYSTEM_PROMPTS.get("path_sort");
        String userPrompt = "当前路径知识点列表（按拓扑序）：\n"
                + kpIds.toString() + "\n\n"
                + "学生画像：\n"
                + (profileHint != null ? profileHint : "无画像数据") + "\n\n"
        ...
        try {
                ...
                if (choices != null && !choices.isEmpty()) {
                    ...
                    if (message != null) {
                        ...
                        if (content != null && !content.isBlank()) {
                            ...
                            if (json.startsWith("[")) {
                                List<String> sorted = objectMapper.readValue(json,
                                        objectMapper.getTypeFactory().constructCollectionType(List.class, String.class));
                                if (sorted != null && !sorted.isEmpty()) {
                                    log.info("[LlmContentGenerator] 路径排序成功: {} 个节点", sorted.size());
                                    return sorted;
                                }
                            }
                        }
            ...
            log.warn("[LlmContentGenerator] 路径排序 LLM 返回空结果");
            return null;
        } catch (Exception e) {
            log.warn("[LlmContentGenerator] 路径排序 LLM 调用失败: {}", e.getMessage());
            return null;
        }
    }
}
